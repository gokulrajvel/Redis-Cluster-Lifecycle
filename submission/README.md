...
*   **World-Writable Directory Auto-Fix**: If running in a shared/cloud environment (e.g., GitHub Codespaces, Gitpod), the `ansible/` directory may be world-writable. `redis-tool` automatically fixes this on startup by removing the world-write permission bit.
